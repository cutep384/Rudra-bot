...
async function getVoiceReply(text, langCode = "hi-in") {
  ...
  const url = `https://api.voicerss.org/?key=${apiKey}&hl=${langCode}&src=${encodeURIComponent(
    text
  )}`;
  try {
    ...
    try {
      api.getUserInfo(uid, (err, data) => {
        if (err || !data || !data[uid]) {
          return resolve(uid === ownerUID ? "Boss" : "yaar");
        }
        const name = data[uid].name || (uid === ownerUID ? "Boss" : "yaar");
        userNameCache[uid] = name;
        resolve(name);
      });
    } catch (e) {
      console.error("getUserName error:", e.message);
      resolve(uid === ownerUID ? "Boss" : "yaar");
    }
  });
}
...
function detectLanguage(text) {
  ...
  if (["ke haal se", "kaisa se", "ram ram"].some((w) => t.includes(w)))
    return "hr";
...
async function toggleHornyMode(body) {
  ...
  if (body.includes("horny mode on") || body.includes("garam mode on")) {
    hornyMode = true;
    return "Horny mode ON 😈🔥";
  }
  if (body.includes("horny mode off") || body.includes("garam mode off")) {
    hornyMode = false;
    return "Horny mode OFF 😉";
  }
  return null;
}
...
async function toggleRiyaOnOff(body, senderID) {
  ...
  body = body.toLowerCase();
  if (body.includes("riya on")) {
    if (riyaEnabled) return "Already ON Boss! 😉";
    riyaEnabled = true;
    return "Riya ON 😎";
  }
  if (body.includes("riya off")) {
    if (!riyaEnabled) return "Already OFF Boss 😴";
    riyaEnabled = false;
    return "Riya OFF 👋";
  }
  return null;
...
async function toggleGoogleMode(body, senderID) {
  if (senderID !== ownerUID) return null;
  body = body.toLowerCase();
  if (body.includes("google on") || body.includes("riya google on")) {
    if (googleMode) return "Google mode already ON 🔍";
    googleMode = true;
    return "Google mode ON 🤓";
  }
  if (body.includes("google off") || body.includes("riya google off")) {
    if (!googleMode) return "Google mode already OFF 😴";
    googleMode = false;
    return "Google mode OFF 😉";
  }
  return null;
}
...
module.exports.handleEvent = async function ({ api, event }) {
  try {
    ...
    const isTrigger =
      body.toLowerCase().startsWith("riya") ||
      messageReply?.senderID === api.getCurrentUserID();
    if (!isTrigger) return;

    const userName = (await getUserName(api, senderID)) || "yaar";
    ...
    let prompt = googleMode
      ? `Google Search_REQUEST: User asked: "${userMsg}"`
      : `Riya conversation with ${userName}: ${chatHistories[senderID].join(
          "\n"
        )}\nRiya:`;

    const resAI = await axios.post("https://rudra-here-brs2.onrender.com", {
      prompt,
    });
    ...
    if (!googleMode) {
      const gifUrl = await getGIF(
        senderID === ownerUID ? "charming fun" : "cool witty"
      );
      if (gifUrl)
        api.sendMessage(
          {
            ...
            attachment: await axios
              .get(gifUrl, { responseType: "stream" })
              .then((r) => r.data),
          },
          threadID,
          messageID
